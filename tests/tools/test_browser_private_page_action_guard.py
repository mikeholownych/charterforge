...
def _browser_mode(monkeypatch):
    ...
    # Authority must never block these guard tests: stub the permit seam.
    monkeypatch.setattr(
        "hermes_cli.workforce_delegation.authorize_worker_action", lambda **k: None
    )
...
def test_private_page_blocks_state_changing_actions(monkeypatch, tool_call, args):
    monkeypatch.setattr(browser_tool._eval_policy, "_eval_ssrf_guard_active", lambda task_id: True)
    monkeypatch.setattr(browser_tool._eval_policy, "_current_page_private_url", lambda task_id: PRIVATE_URL)
...
def test_click_still_runs_when_current_page_is_public(monkeypatch):
    ...
    monkeypatch.setattr(browser_tool._eval_policy, "_eval_ssrf_guard_active", lambda task_id: True)
    monkeypatch.setattr(browser_tool._eval_policy, "_current_page_private_url", lambda task_id: None)
...
def test_guard_inactive_does_not_block_or_probe(monkeypatch):
    ...
    monkeypatch.setattr(browser_tool._eval_policy, "_eval_ssrf_guard_active", lambda task_id: False)

    def fail_probe(task_id):
        raise AssertionError("_current_page_private_url must not be probed when guard inactive")

    monkeypatch.setattr(browser_tool._eval_policy, "_current_page_private_url", fail_probe)
...
def test_camofox_short_circuits_before_guard(monkeypatch):
    ...
    monkeypatch.setattr(browser_tool._eval_policy, "_eval_ssrf_guard_active", fail_guard)
    monkeypatch.setattr(browser_tool._eval_policy, "_current_page_private_url", fail_guard)
