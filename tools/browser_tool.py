...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    if safety_error is None:
        # Governed workers must hold an exact navigation grant bound to the
        # receiving session AND the destination (URL hash) so a permit cannot
        # be replayed to open a different page.
        _authorize_browser_action(
            "browser.navigate",
            effective_task_id,
            target_resource=(
                "browser-navigation:"
                f"{nav_session_key}:url:"
...
def _authorize_browser_action(
    ...
    Governed workers must hold an exact grant: a browser toolset alone is not
    authority to act. The permit binds to the browser session that will receive
    the operation so it cannot be replayed in another worker's session.
    """
...
def _guarded_action(task_id: Optional[str], action: str, command: str, args: list, ok: Dict[str, Any], err: str) -> str:
    ...
    effective_task_id = _authorize_browser_action(f"browser.{action}", task_id)
...
def browser_console(clear: bool = False, expression: Optional[str] = None, task_id: Optional[str] = None) -> str:
    ...
    effective_task_id = _authorize_browser_action("browser.console", task_id)
    blocked = _blocked_private_page_content(effective_task_id)
    if blocked is not None:
        return blocked
...
def _browser_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    effective_task_id = _authorize_browser_action("browser.eval", task_id)
